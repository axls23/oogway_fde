...
def apply_guest_boost(chunks: list[ScoredChunk], query: str) -> list[ScoredChunk]:
    ...
    GUEST_NAME_BOOST (0.03) is deliberately smaller than SESSION_BOOST
    (0.05): "this guest was already cited earlier in this session" is a
    session-scoped fact with no ambiguity, whereas "the guest's name (or
    a token of it) appears in the query text" is a noisier, string-match
    signal — the query could mention a guest's name in passing, in a
    comparison ("more direct than Shreyas Doshi's usual take"), or match
    a distinctive-but-coincidental token. A smaller cap keeps this boost
    unable to outweigh a materially stronger embedding match on its own,
    while still being enough to break near-ties in favor of the
    explicitly-named guest's episode.
    """
    for c in chunks:
