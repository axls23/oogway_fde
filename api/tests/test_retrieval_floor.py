...
from app.services.retrieval import ScoredChunk, apply_floor, apply_session_boost


def _chunk(chunk_id: int, episode_id: int, score: float) -> ScoredChunk:
    return ScoredChunk(
        chunk_id=chunk_id,
        episode_id=episode_id,
        episode_title=f"Episode {episode_id}",
        guest=f"Guest {episode_id}",
